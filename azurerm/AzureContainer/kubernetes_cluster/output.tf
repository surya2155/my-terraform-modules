...
output "az_k8s_identity" {
  value = {
    principal_id = azurerm_kubernetes_cluster.az_kubernetes_cluster.identity[0].principal_id
    tenant_id    = azurerm_kubernetes_cluster.az_kubernetes_cluster.identity[0].tenant_id
  }
  description = "Managed service identity properties for the azure aks"
}