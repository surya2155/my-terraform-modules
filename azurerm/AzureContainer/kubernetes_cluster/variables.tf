...
variable "automatic_channel_upgrade" {
  description = "automatically upgrade kubernetes version"
  type        = string
  default     = "none"
}
...
variable "api_server_access_profile_enabled" {
  description = "If true cluster API server will be exposed only on internal IP address and available only in cluster vnet."
  type        = bool
  default     = false
}
...
variable "local_account_disabled" {
  description = "if local accounts disabled"
  type        = bool
  default     = true
}
...
# key_vault_secrets_provider config vars
variable "secret_rotation_enabled" {
  description = "Sets up the csi secrets provider add on aks"
  type        = bool
  default     = false
}

variable "secret_rotation_interval" {
  description = "Sets up the secret_rotation_interval for aks"
  type        = string
  default     = "2m"
}

# oms_agent config vars
variable "oms_agent" {
  description = "Sets up oms_agent add on aks"
  type        = any
  default     = {}
}
